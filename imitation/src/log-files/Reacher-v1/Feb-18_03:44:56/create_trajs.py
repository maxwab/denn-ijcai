...
def apply_learnt_policy_for_episode(sess, env, output_action_node, scaled_observation_node, offset, scale):
...
def apply_learnt_policy(episodes, sess, env, output_action_node, scaled_observation_node, offset, scale):
    total_steps = 0
    trajectories = []
...
def create_trajs(env_name: str, episodes):
    """
    ...
    trajectories = apply_learnt_policy(episodes, sess, env, output_action_node, scaled_observation_node, offset, scale)

    import dill
    with open('trajs-{}.dill'.format(episodes), 'wb') as fd:
...
if __name__ == '__main__':
    ...
    create_trajs(args.env_name, args.n)
