...
p = Path('log/train:mnist_repulsive:fashionmnist_l:0.05_b:10')
...
if dataset.lower() == 'mnist':
    ...
    print('Bad dataset: can\'t load dataset {}'.format(dataset))
...
if dataset in ['mnist', 'emnist', 'kmnist', 'notmnist']:
    base_model = models.NNMNIST(28 * 28, 10)
    reshape = lambda x: x.view(-1, 28 * 28)
else:
    raise ValueError('Can\'t load model for test dataset {}'.format(dataset))
...
if save:
    ...
    h5_filename = path_stats / '{}_final.h5'.format(dataset.lower())
    ...
    if dataset == 'mnist':
        ...
        h5_filename = path_stats / 'acc_{}_final.h5'.format(dataset.lower())
        h5f = h5py.File(h5_filename, 'w')
