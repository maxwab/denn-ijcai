...
from tools import compute_norm_fac
...
def train(args, experiment=None, device=None):
    ...
    if args.bootstrapping:
        new_mapping = np.random.choice(np.asarray(train_dset.mapping), size=train_dset.length)
    ...
    for epoch in tqdm(range(args.n_epochs), desc='epochs'):
        ...
        for j, batch_raw in enumerate(_tqdm):
            ...
            if args.repulsive is not None:
                ...
                kwargs = {'lambda_anchoring': args.lambda_anchoring, 'prior': prior, 'fac_norm': fac_norm}
            else:
                kwargs = {}
    ...
    try:
        ...
        if not Path.exists(model_path):
            torch.save(net.state_dict(), model_path)
        if args.beta > 0.0:
            prior_path = savepath / dirname_priors / '{}_{}epochs.pt'.format(model_name, epoch + 1)
...
def main():
    ...
    parser.add_argument('--lambda_anchoring', type=float, default=0.0)
