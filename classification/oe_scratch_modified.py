...
from models.convnet import ConvNet, NNMNIST
import dataset
...
parser.add_argument('--lambda_oe', type=float, default=0.5, help='Value of the trade-off parameter.')
...
# Changes that I made
# Changing the learning rate to the default value of Adam, to use the same as the deep ensemble baseline.
...
full_train_data_in = dset.MNIST('../../../datasets/MNIST', train=True, transform=tfms)
test_data = dset.MNIST('../../../datasets/MNIST', train=False, transform=tfms)
num_classes = 10

# Splitting the data into train and validation
train_data_in, val_data_in, _, _ = dataset.train_valid_split(full_train_data_in, split_fold=10, random_seed=args.dataset_seed)
...
tfms_fashionmnist = trn.Compose([trn.ToTensor(), trn.Normalize((.2859,), (.3530,))])
train_data_out = dset.FashionMNIST('../../../datasets/FashionMNIST', train='True', download=False, transform=tfms_fashionmnist)
...
print('Training using lambda value: {}'.format(args.lambda_oe))

# Create model
#net = ConvNet()
net = NNMNIST(28*28, 10)
...
# We also change the optimizer as we are leaning on the baseline from the deep ensembles.
...
# No scheduler, similar to the deep ensemble
...
def train():
    ...
    for in_set, out_set in zip(train_loader_in, train_loader_out):
        ...
        data, target = prepr(data).to(device), target.to(device)
        ...
        loss += args.lambda_oe * -(x[len(in_set[0]):].mean(1) - torch.logsumexp(x[len(in_set[0]):], dim=1)).mean()
