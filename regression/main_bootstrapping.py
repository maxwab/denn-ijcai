...
parser.add_argument('--per_data', type=float, default=1.0)
parser.add_argument('--dropout_rate', type=float, default=0.0)
parser.add_argument('--batch_size', type=int, default=10)
parser.add_argument('--verbose', action='store_true')
parser.add_argument('--comet', action='store_true')
parser.add_argument('--save_folder', type=str, default='log/bootstrapping')
...
model_name = 'bootstrapping_per-data:{:.3f}_dr:{:.3f}'.format(args.per_data, args.dropout_rate)
...
# Bootstrapping
n_el = np.floor(args.per_data * X.size(0)).astype(np.uint32)  # We fix the size of the bootstrap
